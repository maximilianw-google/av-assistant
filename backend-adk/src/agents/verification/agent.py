...
from .tools import load_image_artifact_tool
...
async def add_available_streetview_images_to_prompt(
  ...
  try:
    available_files = await list_artifacts(callback_context)
      ...
      llm_request.contents.append(
          types.Content(
              ...
              parts=[
                  types.Part(
                      text=(
                          "Note: No Street View images are currently available"
                          " for analysis."
                      )
                  )
              ],
    ...
    if llm_request.contents and llm_request.contents[-1].role == "user":
      ...
      # create a new content block for the images, perhaps attributed to 'model'
    ...
    for file in available_files:
      if file.startswith("streetview"):
        try:
          ...
          print("Error loading artifact {file}: {load_error}")
...
root_agent = LlmAgent(
    ...
    instruction=("""
        ...
        **Mandatory Aspects of Analysis & Specific Instructions:**
        ...
        2.  **`aspect_name`: "License and Registration"**
        ...
        3.  **`aspect_name`: "Business Name Consistency"**
        ...
        4.  **`aspect_name`: "Business Address Verification (from Business Details)"**
        ...
        5.  **`aspect_name`: "Business Invoice Content Review"**
        ...
        6.  **`aspect_name`: "Business Card Review (Front & Back)"**
        ...
        7.  **`aspect_name`: "Vehicle Registration Document Review"**
        ...
        8.  **`aspect_name`: "Service Vehicle Images Review (Completeness, Branding & License Plate)"**
        ...
        9.  **`aspect_name`: "Business Location Images Review (Address Display & Signage)"**
        ...
        10.  **`aspect_name`: "Utility Bill Review (Presence, Recency & Details)"**
        ...
        11. **`aspect_name`: "Tools & Equipment Images Review (Compliance, Relevance & Verification Item)"**
        ...
        12. **`aspect_name`: "Inter-Document Consistency"**
        ...
        13. **`aspect_name`: "Overall Information Coherence"**
